...
const accountsKey = 'angular-15-signup-verifcation-boilerplate-accounts';
...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function authenticate() {
            ...
            const account = accounts.find(x => x.email === email && x.password === password); //isverified nass if na
...
function register() {
    ...
    if (accounts.find(x => x.email === account.email)) {
        // display alert and already registered "email" in alert
        setTimeout(() => {
            alertService.info(
